...
namespace SecondSightSouthendEyeCentre
{
    public class INSERTDAL
    {
        ...
        public bool AddPatient(Patitent patitent)
        {
            ...
            cmdAddPAtient.Parameters.AddWithValue("@Pat_Image", patitent.Pat_Image);
            cmdAddPAtient.Parameters.AddWithValue("@Month", patitent.Month);
            conString.Open();
        ...
        {
            SqlCommand cmd = new SqlCommand("Insert into Appointment values(@AppointmentType,@Name,@Mobile,@Address,@DoctorsId,@Time,@Comments,@PatCode,@IsAttented,@IsConfirmed,@IsNew,@IsCanceled,@ProcedureRateId,@IsExamineComplete)", conString);
            ...
            cmd.Parameters.AddWithValue("@ProcedureRateId", appoints.ProcedureRateId); 
            cmd.Parameters.AddWithValue("@IsExamineComplete", appoints.IsExamineComplete); 
            conString.Open();
        ...
        public bool InitSetEmployeePass(EmployeeLog employeeLog)
        {
            SqlCommand cmdAddPAtient = new SqlCommand();
            cmdAddPAtient.CommandText = "sp_InitSetEmployeePass";
            cmdAddPAtient.CommandType = System.Data.CommandType.StoredProcedure;
            cmdAddPAtient.Connection = conString;
            cmdAddPAtient.Parameters.AddWithValue("@EmployeeId", employeeLog.EmployeeId);
            cmdAddPAtient.Parameters.AddWithValue("@pass", employeeLog.Password);
            conString.Open();
            int result = cmdAddPAtient.ExecuteNonQuery();
            conString.Close();
            if (result > 0)
        ...
        public bool SendExcepToDB(string exceptionMsg, string exceptionType, string url, string exceptionSource)
        {
            SqlCommand comSendExcepToDB = new SqlCommand("sp_InsertExceptionLoggingToDataBase", conString);
            comSendExcepToDB.CommandType = System.Data.CommandType.StoredProcedure;
            comSendExcepToDB.Parameters.AddWithValue("@ExceptionMsg", exceptionMsg);
            comSendExcepToDB.Parameters.AddWithValue("@ExceptionType", exceptionType);
            comSendExcepToDB.Parameters.AddWithValue("@ExceptionURL", url);
            comSendExcepToDB.Parameters.AddWithValue("@ExceptionSource", exceptionSource);
            if (conString.State == System.Data.ConnectionState.Closed)
            {
                conString.Open();
            }
            int result = comSendExcepToDB.ExecuteNonQuery();
            conString.Close();
            if (result > 0)
                return true;
            return false;
