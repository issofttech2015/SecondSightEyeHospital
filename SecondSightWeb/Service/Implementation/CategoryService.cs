...
namespace SecondSightSouthendEyeCentre.Service.Implementation
{
    public class CategoryService : ICategoryService
    {
        ...
        public IEnumerable<Category> GetCategoryByMaster(int id)
        {
            return baseDB.GetMany(x => x.CategoryMasterId == id).ToList();
        }

        public IEnumerable<Category> GetCategoryByName(string name)
        {
            return baseDB.GetMany(x => x.ItemName == name).Distinct().ToList();
        }
